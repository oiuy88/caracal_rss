...
# Disguise the script as a normal web browser to bypass CIRCABC's firewall
HEADERS = {
    "accept": "application/json",
    "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36",
    "Referer": "https://circabc.europa.eu/ui/group/a0b483a2-4c05-4058-addf-2a4de71b9a98/library/84998de9-01ff-4434-b566-85367d2fae5b"
}

def fetch_node_children(node_id):
    url = f"https://circabc.europa.eu/service/circabc/spaces/{node_id}/children?language=en&guest=true&limit=100&page=1&order=modified_DESC&folderOnly=false&fileOnly=false&skipExpiredItems=true"
    
    try:
        response = requests.get(url, headers=HEADERS, timeout=15)
        if response.status_code != 200:
            print(f"Failed to fetch {node_id}: HTTP {response.status_code}")
            return []
            
        data = response.json()
        
        # Safely extract the list of items based on CIRCABC's JSON structure
        ...
        print(f"Unknown data structure for {node_id}: {list(data.keys()) if isinstance(data, dict) else type(data)}")
        return []
    except Exception as e:
        print(f"Error fetching {node_id}: {str(e)}")
        return []

def get_all_documents(node_id, depth=0, max_depth=5):
    # Prevent infinite loops if folders are nested too deeply
    if depth > max_depth:
        return []
    ...
    for item in items:
        ...
        is_folder = (
            item.get('isFolder') is True or 
            item.get('type') == 'space' or 
            'folder' in str(item.get('mimeType', '')).lower()
        )
...
def build_rss():
    print("Crawling folders recursively...")
    ...
    print(f"\n--- FOUND {len(all_documents)} TOTAL DOCUMENTS. TOP 10 RECENT: ---")
    for doc in latest_documents[:10]:
        print(f"- {doc.get('name', 'Unnamed')} (Modified: {doc.get('modified', 'Unknown')})")
    print("----------------------------------------------------------\n")
